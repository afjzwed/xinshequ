...
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    private WeakReference<View> titleView;
    ...
    public boolean layoutDependsOn(CoordinatorLayout parent, RecyclerView child, View dependency) {
        if (dependency != null && dependency.getId() == R.id.toolbar_private) {
            titleView = new WeakReference<View>(dependency);
        }
        if (dependency != null && dependency.getId() == R.id.rl_header) {
...
//        KLog.i("dependency.getHeight()" + dependency.getHeight());
//        KLog.i("dependency.getTranslationY()" + dependency.getTranslationY());
        if (dependency.getHeight() + dependency.getTranslationY() > getTitleView().getHeight()) {
//            KLog.i("设置全透明颜色...............");
            getTitleView().setBackgroundResource(R.color.color_00000000);
        }
        if (dependency.getHeight() + dependency.getTranslationY() <= getTitleView().getHeight()) {
            double rate = (getTitleView().getHeight() - (dependency.getHeight() + dependency.getTranslationY())) / getTitleView().getHeight();
//            KLog.i(rate);
            getTitleView().setBackgroundColor(Color.argb((int) ((255) * rate), (int) ((255) *
                    rate), (int) ((213) * rate), (int) ((35) * rate)));
        }
...
//        KLog.i("依赖view高度:" + getDependentView().getHeight());
